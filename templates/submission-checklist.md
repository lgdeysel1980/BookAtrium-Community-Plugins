...
- [ ] `pluginApiVersion` is `"2.0"` for new submissions (legacy `"1.0"` / `"1.1"` only for existing API packages)
...
- [ ] `package.fileName` ends in `.bookplugin` for API 2.0 (legacy: `.bookapp-plugin` / `.bookmetadata-plugin`)
